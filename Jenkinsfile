pipeline {
  ...
  stages {
    ...
    stage('Config target') {
      ...
      steps {
        ...
        sh (returnStdout: false, script: '''
          ...
          kubectl exec --namespace kube-system mysql-0 -- mysql -h 127.0.0.1 -uroot -p$PASSWORD -P3306 -e "create database if not exists sphinx_coininfo;"
    ...
    stage('Unit Tests') {
      ...
      steps {
        sh (returnStdout: false, script: '''
          devboxpod=`kubectl get pods -A | grep development-box | awk '{print $2}' | head -n 1`
    ...
    stage('Release docker image for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep phinx-coininfo| grep $tag
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Release docker image for production') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep sphinx-coininfo | grep $tag
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
            TAG=$tag DOCKER_REGISTRY=$DOCKER_REGISTRY make release-docker-images
